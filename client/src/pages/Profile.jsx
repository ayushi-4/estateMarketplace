...
const dispatch = useDispatch();
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'> 
      <h1 className='text-4xl font-extrabold text-center my-7'> Profile</h1>
      <form onSubmit ={handleSubmit} className='flex flex-col gap-4'>
        <input onChange={(e)=> setFile(e.target.files[0])} type='file' ref={fileRef} hidden accept='image*'/>
        <img onClick={()=> fileRef.current.click()} 
        src ={formData.avatar || currentUser.avatar} alt="profile"
        className='rounded-full h-44 w-44 object-cover cursor-pointer
        self-center  transition ease-in-out delay-150 bg-rose-500 hover:-translate-y-1 hover:scale-110 hover:bg-rose-300 duration-300 ' />
...
<p className='text-sm self-center'>
          {fileUploadError ? (
            ...
            <span className='text-blue-950 text-xl'><b>Image successfully uploaded! </b></span>
          ) : (
            ''
          )}
        </p>
